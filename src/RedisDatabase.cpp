...
Simple text based persistance, each line in the file is one data store
...
//Key value operations set and get
...
bool RedisDatabase::rename(const std::string& oldKey,const std::string& newKey){
    ...
    if(itLv != list_store.end()){
        ...
        list_store.erase(itLv);
        found  = true;
    }

    auto itHv = hash_store.find(oldKey);
    if(itHv != hash_store.end()){
        hash_store[newKey] = itHv->second;
        hash_store.erase(itHv);
        found  = true;
    }
...
bool RedisDatabase::lindex(const std::string& key, int index, std:: string& value){
    ...
    int lsize = static_cast<int>(lst.size());
    if(index < 0){
        index = lst.size() + index;
    }
    if(index < 0 || static_cast<ssize_t>(index) >= lsize){
        return false;
    }
